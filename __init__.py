...
DOMAIN = "bisecur2mqtt"
WATCHDOG_INTERVAL = 30  # Проверять каждые 30 секунд

service_task = None
watchdog_enabled = True

async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry):
    """Запуск Bisecur2MQTT при старте HA."""
    ...
    # Включаем Watchdog (если включен в настройках)
    if watchdog_enabled:
...
async def check_service_status(_):
    """Watchdog: проверяет, работает ли сервис, и перезапускает при необходимости."""
    global service_task
